...
static ImGuiKey ImGui_ImplWin32_GetUntranslatedKey(const ImGuiKeyData* key_data)
{
    ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData();

    if (key_data->Keycode < 0 || key_data->Keycode >= IM_ARRAYSIZE(bd->UntranslatedKeyMap))
        return ImGuiKey_None;

    return bd->UntranslatedKeyMap[key_data->Keycode];
}
